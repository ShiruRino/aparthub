<form method="GET" action="{{ $action ?? url()->current() }}" class="visitor-table-filters">
    @if (! empty($showStatus ?? true))
        <select name="status" aria-label="Visitor status filter">
    ...
    @if (! empty($showSource ?? false))
        <select name="registration_source" aria-label="Registration source filter">
    ...
    @if (! empty($showResident ?? false))
        <select name="resident_id" aria-label="Resident filter">
